...
public static class Queries
{
    ...
    public static IStatement Where(this IQuery query, params (string, object)[] condition_values)
    {
        ...
        var data_params = condition_values.Select(data => ConvertDataType(data.Item1, data.Item2, data.Item2.GetType()))
            .ToList();
    ...
    public static IExecutor Insert<T>(this IQuery query, T obj)
    {
        ...
        var data_params = column_data.Select(data => ConvertDataType(data.PropertyName, data.Value, data.ProprtyType))
            .ToList();
        return new MySqlExecutor(query, data_params);
    }
