...
public sealed class ModuleConfiguration<TConfiguration>(string name)
    : ModuleConfiguration(name) where TConfiguration : class, IConfig, new()
{
    ...
    public override bool UnloadConfiguration(string dataPath, string fileName)
    {
        ...
        ConfigurationManager.Instance.Get(x => x.ConfigName == name, out var conf);
        ConfigurationManager.Instance.Remove(conf);
